...
`;
  ...
  extractActionItems: (agenda, goal, content) => {
    return `以下の会議の内容を見て、アクションアイテムの一覧をリストにしてJSON形式で返して。
...
\`\`\`json
[
  "新製品ローンチに向けたマーケティングプランの草案作成",
  "ユーザーテスト用プロトタイプの動作確認",
  "今期売上予測データの再集計とグラフ作成",
  "システム負荷テストの実施スケジュール調整",
  "	顧客向けFAQページの更新案を作成"
]
`;
  },
...
\`\`\`mermaid
graph TD
    A[Appleの将来性の議論] --> B(会議の内容);
    B --> C[\"なくなってから毎日\"];
    B --> D[\"ビジネス サイダー\"];
    B --> E[\"驚くほど 骨盤の発表\"];
    B --> F[\"年初から株価は 18%も下がっている\"];
    B --> G[\"さらにとどめですね\"];
    B --> H[\"1番 今旬な ガジェットジャーナリストの MK\"];
    A --> I{会議のゴール: Appleの将来性の議論};
\`\`\`
`;
  },
